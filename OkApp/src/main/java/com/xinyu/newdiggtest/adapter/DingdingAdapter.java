...
import com.xinyu.newdiggtest.bean.DingdingBean;
...
import com.xinyu.newdiggtest.inter.onPopClick;
...
public class DingdingAdapter extends BaseMultiItemQuickAdapter<DingdingBean, BaseViewHolder> {


    public DingdingAdapter(@Nullable List<DingdingBean> data) {
        super(data);
        addItemType(5, R.layout.item_ding_head);//标题
        addItemType(4, R.layout.item_daka1);//好友动态
        addItemType(3, R.layout.item_todo);//代办
        addItemType(2, R.layout.item_dingd_msg);//消息
        addItemType(1, R.layout.item_dingd_sx);//私信
    }
    ...
    @Override
    protected void convert(BaseViewHolder helper, DingdingBean data) {

        switch (data.getItemType()) {
            case 5:
                helper.setText(R.id.id_text, data.getFrom_name());

                break;

            case 4://关注好友

                FollowListBean focuBean = data.getFocusBean();
                putData(helper, focuBean);
            ...
            case 1://私信
                ...
                helper.setText(R.id.time, convertTime(item.getCreate_time()));

                if (!MyTextUtil.isEmpty(item.getLatstMsg())) {
                    helper.setText(R.id.tv_latestmsg, AppUtils.convertMsg(item.getLatstMsg()));
                }
    ...
    private void putData(BaseViewHolder helper, final FollowListBean item) {
        ...
        Picasso.with(mContext).load(item.getUserBean().getHead()).error(R.drawable.icon_no_download).
    ...
    private String convertTime(String create_time) {
        if (!MyTextUtil.isEmpty(create_time) && create_time.length() > 9) {
            String neTime = create_time.substring(create_time.length() - 10, create_time.length() - 5);
            return neTime;
        }
